...
public class ExcelUtility {
	...
	public static void excelUtility() throws FileNotFoundException {
		String path = System.getProperty("user.dir")+"\\TestData\\NewsAround_Cognizant.xlsx";
		file = new FileOutputStream(path);
		wb=new XSSFWorkbook();
		s=wb.createSheet("Sheet1");
	}
	
	public static void output(List<String> news, List<String> second_news, List<String> SecondNews) throws FileNotFoundException {
		...
		for(int row=1; row<7; row++) {
			...
			rows.createCell(0).setCellValue(news.get(row-1));
		}
		...
		for(int row=11; row<16; row++) {
			XSSFRow rows = s.createRow(row);
			rows.createCell(0).setCellValue(second_news.get(row-11));
		}
		...
		for(int row=19; row<24; row++) {
			XSSFRow rows = s.createRow(row);
			rows.createCell(0).setCellValue(SecondNews.get(row-19));
		}
